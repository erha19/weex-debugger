...
function openDebugger(deviceId) {
    ...
    device.debuggerWindow = window.open(`/debugger.html?channelId=${device.channelId}`, `debugger${device.channelId}`);
    var $WeexInspectorProxy=`ws://${location.host + '/debugProxy/inspector/' + device.channelId}`;
...
function openInspector(deviceId) {
    var device = findDevice(deviceId);
    device.inspectorWindow = window.open(`/inspector/inspector.html?ws=${location.host + '/debugProxy/inspector/' + device.channelId}&remoteFrontend=1`, `inspector${device.inspectorSessionId}`);
