  ...
  static check () {
    Object.keys(_routerInstances).forEach((id) => {
      if (Object.keys(_routerInstances[id].hubs).length === 0) {
        console.warn('[Mlink Warning] Router[' + id + '] do not has any hub.make sure your id is correct');
      }
    });
  }
  ...
  _event (signal) {
    switch (signal.type) {
      case Router.Event.TERMINAL_JOINED:
        Logger.log(`[terminal]${signal.hubId}-${signal.terminalId} joined`);
        if (signal.channelId) {
          const channel = this.channelMap[signal.channelId];
          if (channel) {
            Logger.log(`[channel]${signal.hubId}-${signal.terminalId} join channel ${signal.channelId}`);
      ...
      case Router.Event.TERMINAL_LEAVED:
        Logger.log(`[terminal]${signal.hubId}-${signal.terminalId} leaved`);
        if (signal.channelId) {
          const channel = this.channelMap[signal.channelId];
          if (channel) {
            Logger.log(`[channel]${signal.hubId}-${signal.terminalId} leave channel ${signal.channelId}`);
