    ...
    switch (signal.type) {
      case Router.Event.TERMINAL_JOINED:
        ...
        if (signal.channelId) {
          ...
          if (channel) {
            logger.verbose(`[channel]${signal.hubId}-${signal.terminalId} join channel ${signal.channelId}`);
      ...
      case Router.Event.TERMINAL_LEAVED:
        ...
        if (signal.channelId) {
          const channel = this.channelMap[signal.channelId];
          if (channel) {
            logger.verbose(`[channel]${signal.hubId}-${signal.terminalId} leave channel ${signal.channelId}`);
