...
class Channel {
  ...
  findAll () {
    return Object.keys(this.hubMap).map(hid => ({ hubId: hid, terminalId: this.hubMap[hid] }));
  }
  ...
  has (hubId, terminalId) {
    return this.hubMap[hubId] && (!terminalId || this.hubMap[hubId] === terminalId);
  }
  ...
  join (hubId, terminalId, forced) {
    if (this.hubMap[hubId]) {
      if (forced) {
        ...
        throw new Error('Join failed! There already exist one terminal in the same hub');
      }
    }
    else {
  ...
  leave (hubId, terminalId) {
    if (this.has(hubId, terminalId)) {
      this.cache = this.cache.filter(c => c.hubId !== hubId || terminalId && c.terminalId !== terminalId);
