...
class Filter {
  constructor (handler) {
    ...
    this.isGeneratorFunction = Object.prototype.toString.call(handler) === '[object GeneratorFunction]';
  }
  run (message, next) {
...
const filterChain = [];
filterChain.push(new Filter(function (message, next) {
  message.from = 1;
  message.to = 2;
  next();
}));
...
Filter.resolveFilterChain({}, filterChain).then(function (data) {
  console.log(2, data);
}, function (err) {
  console.log(err);
});
