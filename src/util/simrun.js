...
function getRuntime () {
  const runtimes = JSON.parse(execSync('xcrun simctl list runtimes --json').toString().trim());
...
function loadApp (appUrl) {
  return new Promise((resolve, reject) => {
    if (/^(https?|ftp)/.test(appUrl)) {
      ...
      const url = schema[0];
      const location = schema[1] || path.basename(appUrl, path.extname(appUrl)) + '.app';
      const targetPath = path.join(process.env['HOME'], '.simrun/', path.join('weexplayground', location));
